...
{
    Vec3 IVec = line.end - line.start;  // Line Vector
...
Interval3D MathUtils::GetInterval(const AABB3D& aabb, const Vec3& axis)
{
    ...
    for (int i = 1; i < 8; ++i)
    {
        ...
        result.max = min(result.max, projection);
    }
...
bool MathUtils::PointInTriangle(const Point3D& p, const Triangle3D& t)
{
    ...
    Vec3 normPCA = c.Cross(a);  // Normal of PCA (u)
    Vec3 normPAB = a.Cross(b);  // Normal of PAB (u)
...
Vec3 MathUtils::Barycentric(const Point3D& point, const Triangle3D& triangle)
{
    // TODO
    return Vec3();
}
