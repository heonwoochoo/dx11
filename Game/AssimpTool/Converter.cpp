...
void Converter::ReadAssetFile(wstring file)
{
	...
	assert(std::filesystem::exists(p));	// 파일이 정상적으로 읽히는지 확인
...
void Converter::ReadModelData(aiNode* node, int32 index, int32 parent)
{
	...
	// Local (Root) Transform : Root를 기준으로하는 Transform으로 변환
...
void Converter::ReadMaterialData()
{
	for (uint32 i = 0; i < _scene->mNumMaterials; ++i)
	{
		...
		// specular의 4번째 값은 연산 과정에서 몇 제곱을 할건지에 관한 값임 (효과를 얼만큼 줄건지?)
...
string Converter::WriteTexture(string saveFolder, string file)
{
	...
	if (srcTexture)
	{
		...
		else
		{
			...
			// Save To File
			hr = DirectX::SaveToDDSFile(*img.GetImages(), DirectX::DDS_FLAGS_NONE, Utils::ToWString(pathStr).c_str());
			CHECK(hr);
		}
	}
	else
