...
struct VS_IN
{
    ...
    float4 blendWeights : BLEND_WEIGTHS;
    // INSTANCING
    uint instanceID : SV_INSTANCEID;    // 몇번째 인스턴스인지를 나타내는 데이터
    matrix world : INST;
};
...
VS_OUT VS(VS_IN input)
{
    ...
    //output.position = mul(input.position, BoneTransforms[BoneIndex]);   // Model의 root기준 좌표로 넘어가는 단계
